...
class DB:
    ...
    def unravelFoulDF(self, index):
        ...
        foul_df = foul_df.fillna({'elapsed_plus' : 0, 'player2' : 0})
        ...
        foul_df = pd.merge(foul_df,
            ...
            how = 'inner', left_on = 'player1', right_on = 'player_api_id'
        ).drop('player_api_id', axis = 1).rename(columns = {'player_name' : 'player1_name_foul'})

        foul_df = pd.merge(foul_df,
            self.player[['player_api_id', 'player_name']],
            how = 'inner', left_on = 'player2', right_on = 'player_api_id'
        ...
        foul_df = foul_df.sort_values(by = 'elapsed_foul')
